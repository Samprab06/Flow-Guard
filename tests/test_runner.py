...
from runner.runner import run_trial
...
class RunnerTests(unittest.TestCase):
    def test_records_success_in_isolated_directory(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("runner.runner.subprocess.run", return_value=completed):
    ...
    def test_records_timeout(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            config = root / "config.json"
            config.write_text("{}")
            with patch("runner.runner.subprocess.run", side_effect=__import__("subprocess").TimeoutExpired(["x"], 1)):

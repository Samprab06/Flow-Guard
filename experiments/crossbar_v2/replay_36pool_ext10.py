...
Design (predeclared extension; exact frozen oracle/methods/budget/init/models
  from OFFLINE-SEQUENTIAL-REPLAY-36POOL):
  * Seeds: 101, 103, 107, 109, 113, 127, 131, 137, 139, 149 (exact).
    Legacy seeds 11, 29, 47 kept separate (not rerun here; combined
    downstream only).
...
Usage:
  .venv/ml/bin/python experiments/crossbar_v2/replay_36pool_ext10.py
  Also writes combined 13-seed artifacts by merging legacy
  replay_36pool_offline_results.json with the 10 new seeds
  (legacy traces copied verbatim, never rerun).
"""
...
OUT_DIR = ROOT / "experiments" / "crossbar_v2" / "replay_36pool_ext10"

LEGACY_DIR = ROOT / "experiments" / "crossbar_v2" / "replay_36pool_offline"
LEGACY_RESULTS = LEGACY_DIR / "replay_36pool_offline_results.json"
COMBINED_DIR = ROOT / "experiments" / "crossbar_v2" / "replay_36pool_combined13"

SEEDS = (101, 103, 107, 109, 113, 127, 131, 137, 139, 149)
LEGACY_SEEDS = (11, 29, 47)
SHARED_INIT_N = 8
ADAPTIVE_N = 16
...
def run_trace(seed: int, method: str, pool: list[dict],
    ...
    if best_qor is not None:
        ...
        cum_total_to_best = float(best_call_rec["cum_total_eda_runtime_s"])
        cum_failed_to_best = float(
            best_call_rec["cum_failed_eda_runtime_s"])
        cum_eda_to_best = cum_total_to_best
        final_metrics = {
        ...
        final_metrics = None
        cum_total_to_best = None
        cum_failed_to_best = None
        cum_eda_to_best = None
    ...
    return {
        ...
        "summary": {
            ...
            "cum_eda_s_to_best": cum_eda_to_best,
            "cum_total_eda_s_to_best": cum_total_to_best,
            "cum_failed_eda_s_to_best": cum_failed_to_best,
...
def main() -> int:
    ...
    payload = {
        "label": "OFFLINE-SEQUENTIAL-REPLAY-36POOL-EXT10",
        "protocol": "exact frozen oracle/methods/budget/init/models from OFFLINE-SEQUENTIAL-REPLAY-36POOL; extension seeds only",
        ...
        "legacy_seeds": list(LEGACY_SEEDS),
    ...
    (OUT_DIR / "replay_36pool_ext10_results.json").write_text(
        json.dumps(_jsonable(payload), indent=2, sort_keys=True) + "\n")

    def _write_table_csv(path, trace_list):
        with open(path, "w",
                  encoding="utf-8") as handle:
            handle.write("seed,method,best_feasible_qor,"
                         "first_feasible_call,"
                         ...
                         "cum_eda_s_to_best,"
                         "cum_total_eda_s_to_best,"
                         "cum_failed_eda_s_to_best\n")
            for trace in trace_list:
                ...
                handle.write(f"{s['seed']},{s['method']},{best},"
                             ...
                             f"{s.get('cum_eda_s_to_best', '')},"
                             f"{s.get('cum_total_eda_s_to_best', '')},"
                             f"{s.get('cum_failed_eda_s_to_best', '')}\n")

    def _write_curves_csv(path, trace_list):
        with open(path, "w",
                  encoding="utf-8") as handle:
    ...
    _write_table_csv(OUT_DIR / "per_seed_method_table.csv", traces)
    _write_curves_csv(OUT_DIR / "curves_best_qor_vs_evals.csv", traces)
    ...
    # Combined 13-seed artifacts: legacy 3 traces copied verbatim + 10 new.
    COMBINED_DIR.mkdir(parents=True, exist_ok=True)
    legacy_payload = json.loads(LEGACY_RESULTS.read_text(encoding="utf-8"))
    legacy_traces = legacy_payload["traces"]
    assert sorted({t["seed"] for t in legacy_traces}) == [11, 29, 47]
    assert len(legacy_traces) == 9
    for t in legacy_traces:  # backfill time-to-best for legacy summaries
        if t["summary"]["best_qor_call"] is not None:
            rec = next(c for c in t["calls"]
                       if c["call"] == t["summary"]["best_qor_call"])
            t["summary"]["cum_total_eda_s_to_best"] = rec[
                "cum_total_eda_runtime_s"]
            t["summary"]["cum_failed_eda_s_to_best"] = rec[
                "cum_failed_eda_runtime_s"]
            t["summary"]["cum_eda_s_to_best"] = rec[
                "cum_total_eda_runtime_s"]
        else:
            t["summary"]["cum_total_eda_s_to_best"] = None
            t["summary"]["cum_failed_eda_s_to_best"] = None
            t["summary"]["cum_eda_s_to_best"] = None
    combined_traces = legacy_traces + traces
    assert len(combined_traces) == 39
    saved_seeds = SEEDS
    try:
        globals()["SEEDS"] = tuple(list(LEGACY_SEEDS) + list(saved_seeds))
        combined_summary = summarize(combined_traces)
    finally:
        globals()["SEEDS"] = saved_seeds
    combined_payload = {
        "label": "OFFLINE-SEQUENTIAL-REPLAY-36POOL-COMBINED13",
        "oracle": str(LEDGER.relative_to(ROOT)),
        "oracle_counts": {"pool_total": 36, "feasible": 6,
                          "distinct_feasible_qor": 2},
        "oracle_best_qor": -0.09306823743826212,
        "oracle_best_ids": ["cb36_002", "cb36_006", "cb36_010"],
        "legacy_seeds": list(LEGACY_SEEDS),
        "extension_seeds": list(saved_seeds),
        "seeds": list(LEGACY_SEEDS) + list(saved_seeds),
        "methods": list(METHODS),
        "budget": {"shared_init": SHARED_INIT_N,
                   "adaptive": ADAPTIVE_N, "total_per_method": TOTAL_N},
        "driver_seeds": "replay seed (matched across methods)",
        ...
        "ps_before": ps_before,
        "ps_after": ps_after,
        "legacy_source": str(LEGACY_RESULTS.relative_to(ROOT)),
        "summary": combined_summary,
        "traces": combined_traces,
    }
    (COMBINED_DIR / "replay_36pool_combined13_results.json").write_text(
        json.dumps(_jsonable(combined_payload), indent=2, sort_keys=True)
        + "\n")
    _write_table_csv(COMBINED_DIR / "per_seed_method_table.csv",
                     combined_traces)
    _write_curves_csv(COMBINED_DIR / "curves_best_qor_vs_evals.csv",
                      combined_traces)
    print(f"combined_traces={len(combined_traces)} "
          f"combined_wins={json.dumps(combined_summary['win_counts'], sort_keys=True)}")
    print(f"combined_out={COMBINED_DIR.relative_to(ROOT)}")
    return 0
